...
rather than in passes over the data, on this dataset it was the slow one *and* the
...
fast, which is memory pressure and model shape, neither of which applies to one matrix
product over 20,640 rows.
...
theoretical 3. Say that last one plainly rather than hide it: at these sizes a
multithreaded BLAS solve is nowhere near its asymptotic regime, so the measured
exponent is meaningless and the time projection built on it is nonsense. The memory
projection is not — **$X^\top X$ alone would need 80 GB** at 100,000 features. You run
out of RAM long before you run out of patience.

Cost is the reason people quote. Rank is the reason that stops you. Duplicate one
column and the matrix has 9 columns at **rank 8**; numpy refused with
`Singular matrix`. Gradient descent walked the same problem to **MSE 0.52432099**,
matching `lstsq` exactly, and put **+0.414810** on each duplicate — the same split
`lstsq` chose, without being told anything about the duplication.
...
Error along an eigendirection of the Hessian with curvature $\lambda$ is multiplied by
$(1-\eta\lambda)$ every step, so the safe range is exactly $\eta < 2/\lambda_{\max}$.
Here that ceiling is **0.4934**. Eight multiples of it, 600 steps each:
...
the loss rule quit after **2,203 steps**, still **6.525e-02** from the exact answer and
**3.207e-04** above the best loss. The gradient rule ran **8,742 steps** and finished
**2.316e-06** away, **4.039e-13** above the best. The loss rule cannot tell "nowhere
left to go" from "barely moving".
...
**14307.4**. Clipping at ±5 touches **282 of 20,640 rows** and brings the max to
**70.8**. Each method then gets the largest step safe for its own gradient estimate,
so this compares batch sizes and not learning rates.
...
because one batch step is a single BLAS call across every core on a matrix that fits in
cache, while 41,250 single-row steps are 41,250 trips through the Python interpreter.
The wobble column is the spread of the remaining gap on a log scale over the last 40%
of each run, so batch's 0.808 is large because that run was still falling through
orders of magnitude, not because it was noisy.

With a constant learning rate, stochastic descent does not converge to a point at all.
It settles into a cloud, and decaying the rate is the only way to shrink it.
...
original units: condition **44.5**, **439 steps**, gap 1.110e-16. Standardised:
condition **1.3**, **81 steps**, gap 3.331e-16.

On all eight features with a shared 20,000-iteration budget it stops being cosmetic:
