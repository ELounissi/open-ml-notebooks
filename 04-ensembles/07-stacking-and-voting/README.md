...
Soft voting did not beat the best member at all.
...
is smaller than the noise. I am not saying stacking is useless — I am saying that on a
well-tuned gradient booster with five look-alike friends, this is how the numbers
...
Test row 69 is the case worth reading. True class SIRA; hard voting said HOROZ 4-2 and
was wrong, soft voting said SIRA and was right. Three of those four HOROZ votes were
cast at **0.508**, **0.520** and **0.607** — barely leaning — while the two SIRA votes
came in at **0.667** and **0.998**. Counting labels throws that away.

The confidence column is the caveat. The decision tree reports **1.000** on every row
it names, and naive Bayes multiplies independent likelihoods it does not have. A loud
...
worst **lost 0.0159**, with only **r = 0.36** between how alike a trio was and what
voting bought it.
...
cancel an error every member makes. Check the correlation matrix before spending an
afternoon on the ensemble.

## The out-of-fold rule, and the leak done on purpose

Fit the base models on the training set and predict that same training set, and this
is what those columns claim about themselves:
...
out-of-fold columns took **16.9 s**. Trained on those, the meta-model claimed 0.9310
and got **0.9207**. Trained on the in-sample columns it claimed **1.0000** and got
**0.9130**. So the leak invents **+0.0793** of reported accuracy no held-out data will
...
same say on every class forever, and can never subtract a member.
`StackingClassifier` reproduced my hand-built version to **0.0000** difference.

## The full board, and the same rule in regression

![Members against ensembles](figures/fig-03-members-vs-ensembles.png)

Stacking **0.9207**, hard vote 0.9185, gradient boosting 0.9185, soft vote 0.9181,
logistic 0.9170, k-NN 0.9148, random forest 0.9100, decision tree 0.8844, naive Bayes
0.7493.
...
earns its cost when the members are genuinely different, when their error correlation
is low, and when a fraction of a point justifies six models in production.
