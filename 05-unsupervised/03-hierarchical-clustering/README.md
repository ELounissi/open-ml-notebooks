...
cluster *is*. And on the real dataset, **plain k-means beat every linkage**, Ward
included, on the same sample.
...
Ward lost to k-means by 0.012 on identical data. Honest, and not a surprise: Ward merges
greedily on the same within-cluster sum of squares k-means minimises directly, so on
round similar-sized clouds the direct optimiser should edge the greedy one. Single
linkage sits near zero because in sixteen standardised dimensions there is always a
chain of near neighbours between any two varieties. Hierarchical clustering does buy one
thing k-means cannot: any distance works, since the algorithm only ever asks for
distances. Average linkage scored **0.318** on Euclidean and **0.300** on Manhattan, and
complete linkage on **cosine reached 0.428**, better than complete on Euclidean.
...
| **Complete and average** | Complete wants similar diameters and one far point sets them; average is the middle option and topped cophenetic correlation at 0.831 |
| **Ward** | Euclidean only. The safe default, and it lost to k-means by 0.012 on the beans |
| **Where to cut** | `fcluster(Z, t, "distance")` states a distance and accepts the count; `fcluster(Z, k, "maxclust")` states a count and hides the distance |
