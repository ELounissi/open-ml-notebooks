...
Exactly zero, not approximately zero. Cosine similarity between the two feature maps
is **0.008** unaligned and **0.969** once one is shifted back. The same shift through
a fixed dense layer gives cosine **0.313** between its outputs. The dense layer has no
way to know the two pictures show the same garment.

The second number is the price:

| | Parameters |
|---|---|
| `Conv2d(16, 32, 3)` | **4,640** |
| Dense layer, same 16×14×14 tensor to 32×14×14 | **19,675,264** |

A ratio of **4,240×**, and the convolution holds 4,640 parameters whether the input is
...
bright-to-dark left to right, negative the other way, near zero on flat ground.

One point of vocabulary: the formula above is cross-correlation. A true convolution
flips the kernel first. Every library does the unflipped version and calls it
convolution, and it makes no difference, because the weights are learned.
...
float64 machine epsilon of 2.220e-16. That is six units in the last place, which is
the only agreement worth claiming. It also confirms the unflipped convention — a
flipped kernel would disagree in the first decimal, not the sixteenth.
...
13; with p=1 it gives 14; k=5, s=3, d=2 gives 7 from a reach of 9.

`same` padding for stride 1 and odd $k$ works out to $p = (k-1)/2$ — one ring for 3×3,
two for 5×5, three for 7×7, all keeping 28×28 where `valid` gives 26, 24 and 22. Two
things the library does quietly: a 4×4 kernel with `padding='same'` pads 2 on one side
and 1 on the other, and `padding='same'` with stride 2 raises `RuntimeError`.

The cost of shrinking shows up when you stack. From 28×28, a 3×3 valid conv can be
stacked **13 times** before the map is 2×2: 28, 26, 24, 22, 20, 18, 16, 14, 12, 10, 8,
6, 4, 2. The padded stack stays at 28 forever. That is why `padding=1` appears on
nearly every 3×3 layer in nearly every architecture — a deep network with no padding
spends its depth budget eating its own image, from the border inwards, so edge pixels
pass through fewer layers than central ones.
...
On the raw image the three look almost alike, which is why the second row exists. On
the edge map max pooling keeps **100%** of every thin edge peak while average pooling
keeps **80.9%** and the strided conv **76.0%** — thin structure fades. None of the
three is right in general. Max pooling detects whether a feature is present at all,
average pooling is gentler and is what global pooling does at the end of a modern
network, and a strided convolution has parameters, so it can learn what to keep.
...
576 non-zero pixels of 4,225.** I use average pooling for the probe because max
pooling routes its gradient to one winner and would undercount a footprint the unit
genuinely has.
...
The exact zero from the top of this page lives here. Because the same weights apply at
every position, moving the input moves the output by the same amount and changes
nothing else — the interior difference is `0.000e+00`, which is a stronger claim than
similarity. The 3.486 figure is the border, where the shift pulls in pixels that were
not there before.
