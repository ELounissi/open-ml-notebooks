...
**The caption on that chart is wrong and I am leaving it visible.** The verdict
string is generated by a branch that compares the gain at 250 images against the
gain at 4,000 and never checks the sign of either. Both are negative, so the
branch fires the sentence written for the case where augmentation helped. The
plotted line is the truth: it sits entirely below zero, and the grey band marking
one standard error is nowhere near it. The same slip reaches the printed summary,
which reports `largest gain: -0.0190`, and the next chart, whose title compares
what "the wrong augmentation costs" against what "the right one gains" and then
prints -0.1635 against -0.0190.
