...
Prior samples, by mean distance to the nearest of the 12,000 training images:
...
model draws nearly the same picture every time, and one average garment is close
to everything. Distance to real data is a blur score as much as a fidelity score.
...
Two heads instead of one cost almost nothing: **472,608** parameters against
**474,672**. Sampling is the problem, and PyTorch names the distinction directly.
...
`.sample()` detaches and the encoder never receives a gradient. The other route,
the score-function estimator, is unbiased too, so the case against it has to be
measured. Estimating $d/d\mu\,\mathbb{E}[z^2]$, where the truth is exactly 2:
...
Both land on the answer. The score-function estimator needs **7.8x as many draws**
to match the other's standard error, which is what decides whether training works
at the batch sizes anyone uses.
...
0.249 at beta = 0, so output still varies with input. The loss climbs
monotonically, 229.453 to 287.705, and tells you none of this on its own.
...
The VAE is smoother on both paths, but on the first by only **0.011**. Smooth
interpolation is this model's headline claim, and at sixteen dimensions it can be
a rounding error. The real difference is upstream, in the sampling table.
